...
class TCGADataset(Dataset):
    def __init__(self, n_train=None, n_test=None, n_bags=None, train = True, transformation=None):
        ...
        # make sure that all slide from same case are in same partition
        self.metadata.sort_values("case_id", inplace=True)
        self.metadata.reset_index(inplace=True, drop=True)
        while self.metadata.loc[self.n_train - 1]["case_id"] == self.metadata.loc[self.n_train]["case_id"] and self.n_train < self.n_bags:
            self.n_train += 1
            self.n_test -= 1

        self.metadata = self.metadata.head(self.n_bags)
        self.metadata_train = self.metadata.loc[:self.n_train]
        self.metadata_test = self.metadata.loc[self.n_train:self.n_train + self.n_test]
        ...
        try:
            with h5py.File(os.path.join(TMP_PATH, f"TCGA_PML_{TASK_ID}.h5"), "w-") as f: # if this fails, the file has already been written
                for i in tqdm(range(self.n_bags), desc="Creating temporary h5py file for faster data access."):
                    n_patches = get_bag_shape(self.slide_paths[self.metadata.loc[i]["slide_id"]])[0]
                    ds = f.create_dataset(self.metadata.loc[i]["slide_id"], (1, n_patches, 768), maxshape=(1, None, 768))
                    ds[0] = get_bag(self.slide_paths[self.metadata.loc[i]["slide_id"]])
    ...
    def __getitem__(self, i):
        ...
        if self.train:
            bag_id = self.metadata_train.loc[i]["slide_id"]
            label = np.array([1]) if self.metadata_train.loc[i]["ajcc_pathologic_t"].startswith("T4") else np.array([0])
        else:
            bag_id = self.metadata_test.loc[self.n_train + i]["slide_id"]
            label = np.array([1]) if self.metadata_test.loc[self.n_train + i]["ajcc_pathologic_t"].startswith("T4") else np.array([0])
